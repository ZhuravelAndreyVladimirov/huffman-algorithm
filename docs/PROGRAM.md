...
- **SRP:** отдельные классы для частот, дерева, таблицы кодов, побитового кодирования/декодирования, записи архива; в Core остаётся и `TreeLayoutCalculator` (опционально для будущей визуализации дерева).
...
| `CodecProgress` | Процент 0–100 и `StageId` для `IProgress` в UI |
...
| `TreeLayoutCalculator` | Координаты узлов/рёбер (ядро; в UI не используется — вместо дерева таблица кодов) |
...
Окно с вкладками **«Работа»** и **«Анализ»**, шрифт **Segoe UI**. Алгоритм сжатия и формат `.huff` не меняются — меняется только представление.

**Вкладка «Работа»**

- **Выбрать файл** — исходник для сжатия; в диалоге фильтры по типам (все файлы, текст, бинарные, изображения). Тот же путь задаётся **перетаскиванием файла** на окно (drag-and-drop).
...
- После успешной операции: **человекочитаемые размеры** (Б / КиБ / МиБ), для сжатия — **доля размера архива от исходника в %**, время выполнения (в т.ч. при распаковке).
- **Копировать сводку** — текст последней успешной операции в буфер обмена.
- **Показать в проводнике** — `explorer /select` для последнего выходного файла.
- Во время сжатия/распаковки: **`ProgressBar`** (стиль блоков), подпись этапа и процента (**`IProgress<CodecProgress>`** из ядра: число 0–100 + идентификатор этапа для локализации), кнопка **«Отмена»** (`CancellationToken`), остальные действия блокируются.
- Сообщение об **ошибке**: краткий текст; по желанию пользователь копирует полный текст исключения в буфер.
- При **тёмной теме приложений Windows** интерфейс подстраивается под тёмные цвета (реестр `AppsUseLightTheme`).

**Вкладка «Анализ»**

- По кнопке **«Обновить анализ»** для файла, выбранного на вкладке «Работа», вызывается **`PreviewAsync`** (частоты и таблица кодов без записи архива).
- **Гистограмма** частот по байтам **0…255** и **`DataGridView`**: для каждого байта с ненулевой частотой — значение, hex, печатаемый символ (ASCII), частота, битовая строка кода Хаффмана и длина кода в битах (сортировка по столбцам).
- Для файлов **больше 50 МиБ** анализ отключается (как и при полном сжатии, данные загружаются в память целиком).

На кнопках сценариев отображаются небольшие **системные иконки** (`SystemIcons`).
